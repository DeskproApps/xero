...
export const useQueryMutationWithClient = <TData = unknown>(
  queryFn: (data: any, client: IDeskproClient) => Promise<TData>
) => {
  const { client } = useDeskproAppClient();

  return useMutation<TData, unknown, unknown, unknown>(
    (data) => (!client ? null : queryFn(data, client)) as Promise<TData>
  );
};
